...
export function PortfolioScene({ progress, scrollProgress, isActive, deviceTier }: PortfolioSceneProps) {
  ...
  const particleCount = deviceTier === 'low' ? 350 : deviceTier === 'medium' ? 500 : 650
  const lineCount = 30
  ...
  const { positions, colors, originalPositions } = useMemo(() => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const radius = 7.5 + Math.random() * 14.5
  ...
  const lines = useMemo(() => {
    ...
    for (let i = 0; i < lineCount; i++) {
      ...
      const radius = 9.5 + Math.random() * 7.5
      lineData.push({
        angle,
        radius,
  ...
  useFrame((state) => {
    ...
    if (groupRef.current) {
      groupRef.current.rotation.y = time * 0.0095
      groupRef.current.rotation.x = Math.sin(time * 0.008) * 0.016
      groupRef.current.position.y = Math.sin(time * 0.11) * 0.03
    }
    ...
    if (linesRef.current) {
      linesRef.current.children.forEach((line, i) => {
        if (line instanceof THREE.Line) {
          ...
          const radius = lineData.radius + Math.sin(time * 1.5 + lineData.phase) * 1.95
          
          const x = Math.cos(angle) * radius
          const y = Math.sin(time * 2 + lineData.phase) * 2.88
          const z = Math.sin(angle) * radius
          
          line.position.set(x, y, z)
          line.rotation.y = time * 0.085
  ...
  return (
    <group ref={groupRef}>
      <Stars radius={85} depth={45} count={450} factor={3.2} saturation={0.11} fade speed={0.45} />
      
      <Points ref={particlesRef} positions={positions} stride={3} frustumCulled={false}>
        <PointMaterial
          transparent
          vertexColors
          size={deviceTier === 'low' ? 0.048 : deviceTier === 'medium' ? 0.034 : 0.024}
      ...
      <mesh rotation={[Math.PI / 2, 0, 0]}>
        <ringGeometry args={[11.5, 0.0145, 128]} />
        <meshBasicMaterial
          color="#3b82f6"
          transparent
          opacity={0.185}
      ...
      <mesh rotation={[Math.PI / 2, 0, 0]}>
        <ringGeometry args={[17.5, 0.0095, 128]} />
        <meshBasicMaterial
          color="#8b5cf6"
          transparent
          opacity={0.095}
      ...
      <mesh rotation={[Math.PI / 2, 0, 0]}>
        <ringGeometry args={[24.5, 0.0075, 128]} />
        <meshBasicMaterial
          color="#06b6d4"
          transparent
          opacity={0.075}
