...
function RobotBody({ isHovered, isActive, position = [0, 0, 0], scale = 1 }: AICoreRobotProps) {
  ...
  useFrame((state) => {
    ...
    // Breathing
    bodyRef.current.scale.y = scale * (1 + Math.sin(t * 2) * 0.02)
    bodyRef.current.scale.x = scale * (1 + Math.sin(t * 2 + Math.PI) * 0.01)

    // Floating motion
    bodyRef.current.position.y = position[1] + Math.sin(t * 1.5) * 0.05

    // Head rotation to LOOK AT the mouse
    if (bodyRef.current.children[0]) {
      const head = bodyRef.current.children[0] as THREE.Group
      const headTargetX = globalMouseX * 0.7
      const headTargetY = globalMouseY * 0.7
    ...
    if (leftPupilRef.current && rightPupilRef.current) {
      ...
      const targetX = globalMouseX * 0.5
      const targetY = globalMouseY * 0.5
    ...
    // Mouth smile
    if (mouthRef.current) {
      const scaleValue = isHovered ? 1.2 : 1
      mouthRef.current.scale.x = THREE.MathUtils.lerp(mouthRef.current.scale.x, scaleValue, 0.1)
    }
  ...
  return (
    <group ref={bodyRef} position={position}>
      ...
      <group position={[0, 0.8 * scale, 0]}>
        ...
        {/* Left eye */}
        <mesh ref={leftEyeRef} position={[-0.15 * scale, 0.05 * scale, 0.38 * scale]} scale={eyeScale}>
        ...
        <mesh ref={leftPupilRef} position={[-0.15 * scale, 0.05 * scale, 0.44 * scale]} scale={eyeScale}>
          <sphereGeometry args={[0.04 * scale, 16, 16]} />
          <meshStandardMaterial color="#1a1a2e" roughness={0.1} />
        </mesh>

        {/* Right eye */}
        <mesh ref={rightEyeRef} position={[0.15 * scale, 0.05 * scale, 0.38 * scale]} scale={eyeScale}>
          <sphereGeometry args={[0.08 * scale, 16, 16]} />
          <meshStandardMaterial color="#ffffff" emissive="#ffffff" emissiveIntensity={0.3} roughness={0.1} />
        </mesh>
        <mesh ref={rightPupilRef} position={[0.15 * scale, 0.05 * scale, 0.44 * scale]} scale={eyeScale}>
          <sphereGeometry args={[0.04 * scale, 16, 16]} />
          <meshStandardMaterial color="#1a1a2e" roughness={0.1} />
        </mesh>

        {/* Mouth */}
        <mesh ref={mouthRef} position={[0, -0.12 * scale, 0.4 * scale]}>
          <torusGeometry args={[0.08 * scale, 0.015 * scale, 8, 16, Math.PI]} />
        ...
        {/* Antenna */}
        <mesh ref={antennaRef} position={[0, 0.45 * scale, 0]}>
          <cylinderGeometry args={[0.02 * scale, 0.02 * scale, 0.2 * scale, 8]} />
      ...
      <mesh position={[0, 0.15 * scale, 0]}>
        <capsuleGeometry args={[0.25 * scale, 0.3 * scale, 16, 32]} />
      ...
      {/* Left arm */}
      <group ref={leftArmRef} position={[-0.35 * scale, 0.3 * scale, 0]}>
        <mesh position={[0, -0.1 * scale, 0]}>
          <capsuleGeometry args={[0.06 * scale, 0.2 * scale, 8, 16]} />
          <meshStandardMaterial color="#6366f1" roughness={0.3} metalness={0.6} />
        </mesh>
      </group>

      {/* Right arm (waving) */}
      <group ref={rightArmRef} position={[0.35 * scale, 0.3 * scale, 0]}>
        <mesh position={[0, -0.1 * scale, 0]}>
          <capsuleGeometry args={[0.06 * scale, 0.2 * scale, 8, 16]} />
          <meshStandardMaterial color="#6366f1" roughness={0.3} metalness={0.6} />
        </mesh>
...
export function AICoreRobot({ isHovered, isActive, position = [0, 0, 0], scale = 1 }: AICoreRobotProps) {
  return (
