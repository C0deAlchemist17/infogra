...
const StudioFooter = () => {
  ...
  const footerSections = [
    { icon: Globe, label: 'Company', links: footerNavigation.company },
    { icon: Send, label: 'Services', links: footerNavigation.services },
    { icon: Globe, label: 'Resources', links: footerNavigation.resources },
  ]
          ...
          <div className="grid lg:grid-cols-4 md:grid-cols-2 gap-10 md:gap-12">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={hasBeenVisible ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.1, duration: 0.8 }} className="lg:col-span-1">
              ...
              <p className="text-body text-text-secondary leading-relaxed mb-8">
                We architect digital experiences where innovative design meets powerful development.
              </p>
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} animate={hasBeenVisible ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.5, duration: 0.8 }}>
              <h3 className="text-h4 font-semibold text-text-primary mb-4">Stay Updated</h3>
              <p className="text-body-sm text-text-secondary mb-5">Subscribe for the latest insights and updates.</p>
              {isSubscribed ? (
                ...
                <form onSubmit={handleSubscribe} className="space-y-3">
                  <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Enter your email" required className="w-full" />
        ...
        <div className="border-t border-border-subtle py-6 md:py-8">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <div className="flex flex-col md:flex-row items-center gap-3 text-center md:text-left">
              <p className="text-small text-text-secondary">© {new Date().getFullYear()} INFOGRA. All rights reserved.</p>
              ...
              <span className="flex items-center gap-1.5 text-small text-text-tertiary">
                Made with <Heart className="w-4 h-4 text-accent-primary" /> in Egypt
              </span>
            </div>
