// Elhamd Exhaustive Discovery + Import API
...
export async function GET(request: NextRequest) {
  ...
  try {
    console.log('=== ELHAMD EXHAUSTIVE DISCOVERY + IMPORT STARTED ===')

    // Phase 1: Discovery
    console.log('\n=== PHASE 1: DISCOVERY ===')
    const discovery = new ElhamdDiscovery(500) // 500ms delay
    ...
    console.log('\n=== PHASE 2: PRODUCT EXTRACTION ===')
    const scraper = new ElhamdScraper(500)
    ...
    const mappedProducts = extractedProducts
      ...
      .map(product => {
        const mappingResult = mapper.mapElhamdProduct(product as any)
    ...
    console.log('\n=== ELHAMD EXHAUSTIVE IMPORT COMPLETE ===')
