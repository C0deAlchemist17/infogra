// Parallel Elhamd import - Multiple entry points to avoid timeout
import { NextRequest, NextResponse } from 'next/server'
import { ParallelElhamdScraper } from '@/lib/importers/parallel-elhamd-scraper'
...
export async function GET(request: NextRequest) {
  ...
  try {
    console.log('=== PARALLEL ELHAMD IMPORT STARTED ===')
    console.log('Scraping from multiple entry points to avoid timeout...')
    
    const scraper = new ParallelElhamdScraper()
    ...
    const uniqueProducts = Array.from(
      new Map(result.products.map(p => [p.id, p])).values()
    )
